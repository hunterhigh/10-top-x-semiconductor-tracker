...
class ReportScopeTests(unittest.TestCase):
    def setUp(self):
        self.accounts = {"op1", "op2", "op3"}
        self.asof = date(2026, 7, 20)

    def test_window_is_closed_d_minus_27_through_d_and_scores_supplied_opinions(self):
        self.assertEqual(report_window(self.asof), ("2026-06-23", "2026-07-20"))
        rows = [row("op1", "2026-06-23"), row("op2", "2026-07-20"), row("op3", "2026-07-20", "bearish")]
    ...
    def test_history_scope_is_monthly_rows_union_unique_top_picks(self):
        monthly = doc("MONTH", [row("op1", "2026-07-20", tweet="1"), row("op2", "2026-07-20", "bearish", "2"), row("op3", "2026-07-20", tweet="3")])
        pick = doc("PICK", [row("op1", "2026-07-20", tweet="4"), row("op1", "2026-07-19", tweet="5")])
    ...
    def test_non_opinion_ids_do_not_expand_monthly_or_favorite_scope(self):
        signal = doc("NEWS", [row("news", "2026-07-20", tweet="9")])
        scope = monthly_history_scope([signal], self.accounts, self.asof)
        self.assertEqual(scope["docs"], [])
        self.assertNotIn("news", scope["top_picks"])
